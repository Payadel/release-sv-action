...
export async function readChangelogSection(
    ...
    if (targetVersion) {
        ...
        targetIndex = headerLines.findIndex(line =>
            new RegExp(`\\b${targetVersion.toLowerCase()}\\b`).test(line.line)
        );
        if (targetIndex < 0) {
            throw new Error(
...
async function autoDetectIsNeedGenerateChangelog(
    changelog_file: string,
    changelogHeaderRegex: RegExp,
    changelogVersionRegex: RegExp,
    inputVersion?: string
): Promise<boolean> {
    const newVersion = await detectNewVersion(inputVersion);
    const newestChangelogVersion = await getNewestChangelogVersion(
        changelog_file,
        changelogHeaderRegex,
        changelogVersionRegex
    );
...
export async function getNewestChangelogVersion(
    changelog_file: string,
    changelogHeaderRegex: RegExp,
    changelogVersionRegex: RegExp
): Promise<string | null> {
